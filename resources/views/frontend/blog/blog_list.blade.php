                    ...
                    <div class="col-md-9">
                        ...
                        @foreach($blogPost as $blog)
                            <div class="blog-post  wow fadeInUp" >
                                ...
                                <h1><a href="{{ route('post.details', $blog->id) }}"> {{ (Session()->get('language') == 'en') ?  $blog->post_title_en : $blog->post_title_ar  }}</a></h1>
                                <span class="date-time">{{ \Carbon\Carbon::parse($blog->created_at)->diffForHumans() }}</span>
                                <p>{!! (Session()->get('language') == 'en') ?  Str::limit($blog->post_details_en , 250) : Str::limit($blog->post_details_ar , 250)  !!}</p>
                                <a href="{{ route('post.details', $blog->id) }}" class="btn btn-upper btn-primary read-more">{{ (session()->get('language') == 'en' ) ? 'Read More' : 'أقرا المزيد' }}</a>
                            </div>
